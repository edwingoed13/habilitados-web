...
// ============ ENDPOINTS DOCENTES-PRE ============

// Endpoint para obtener estadísticas de inscripciones de docentes
app.get('/docentes-pre/api/docentes/estadisticas', async (req, res) => {
  try {
    const connection = await pool.getConnection();

    // Total de inscritos
    const [totalResult] = await connection.query(
      'SELECT COUNT(*) as total FROM inscripcion_docentes'
    );
...
app.get('/docentes-pre/api/docentes/area/:areaId', async (req, res) => {
  try {
    ...
    console.error('Error en consulta por área:', error);
